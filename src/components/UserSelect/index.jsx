...
const UserSelect = (props) => {
			...
			if (users && users.length) {
				users.forEach((val) => {
					...
					newList.push({
						...
						icon: val.userIcon ? (
							<Avatar src={val.userIcon} size={20} />
						) : (
							<Avatar style={{ background: "#5b87fd" }} size={20}>
	 ...
	 */
	const init = () => {
		console.log(2, hasSelectList);
		const filterHasSelectList = hasSelectList.filter((item) =>
			orgList.map((v) => v.id).includes(item.id)
		);
		const filterCheckedList = orgList
			.filter((item) => hasSelectList.map((v) => v.id).includes(item.id))
			.map((item) => item.key);
		let expandKeyList = orgList
	...
	const onCheck = (ck, e) => {
		...
		if (!checked) {
			...
			checkedNodes = checkedNodes.filter(
				(item) => !childIdList.includes(item.id)
			);
		}
		//已选中的人员
		const userList = checkedNodes
		...
		//已选中的key值
		const checkedList = orgList
			.filter((val) =>
				checkedNodes
					.filter(
						(item) =>
							(item.type === "org" && !item.children) ||
							item.type === "user"
					)
					.map((item) => item.id)
					.includes(val.id)
	...
	const handleUniqueArr = (arr) => {
		...
		let finalArr = [];
		new Set(
			arr.map((item) => (item.type ? `${item.type}-${item.id}` : item.id))
		).forEach((v) => {
	...
	const handleSearchSelectUser = () => {
		...
		let newOrgList = orgList.filter(
			(item) => item.type === "user" && item.title.indexOf(value) !== -1
		);
				...
				<Row>
							...
							<div className={styles.selectBorderArea}>
								<div className={styles.selectUserOuterArea}>
									...
									<div className={styles.orgAndUserArea}>
										<div
											className={`${styles.checkboxMainArea} checkboxMainArea`}
										>
					...
					<Col span={12} className={styles.selectColArea}>
						<div className={styles.mainTitleArea}>
							已选
							<span className={styles.hasSelect}>
								{curHasSelectList.length}人
							</span>
						</div>
						<div className={styles.selectBorderOuterArea}>
							<div className={styles.selectBorderArea}>
								{curHasSelectList.map((item, index) => (
									<div
										className={styles.hasSelectItemArea}
										key={item.id}
									>
