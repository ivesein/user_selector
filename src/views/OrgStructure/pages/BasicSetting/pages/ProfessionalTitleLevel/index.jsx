...
import ProfessionalTitleLevelForm from "./components/ProfessionalTitleLevelForm";
import ProfessionalTitleLevelDetail from "./components/ProfessionalTitleLevelDetail";
...
import { TYPE_RPROFESSIONAL_TITLE_LEVEL } from "../config";
// 职称等级
const ProfessionalTitleLevel = (props) => {
	...
	const [hoverRowId, setHoverRowId] = useState(null); // 设置当前鼠标移入的行i
	const [loading, setLoading] = useState(false); //表格数据加载loading
	const [currentRow, setCurrentRow] = useState(false); //当前操作行
	...
	const loadTableData = useCallback(
		async (size, current, search = "") => {
			const params = {
				size,
				current,
				type: TYPE_RPROFESSIONAL_TITLE_LEVEL,
			};
			if (search) {
