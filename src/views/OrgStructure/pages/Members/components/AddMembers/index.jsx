...
const AddMembers = (props) => {
	...
	useEffect(() => {
		// if (
		// 	(props.orgInfo && props.userOptions && props.roleOptions,
		// 	props.jobOptions)
		// ) {
		// 	setFormFields([
		// 		...initFormFields(
	...
	const confirm = async (params) => {
		try {
			...
			let res = await ADD_MEMBERS_API(params);
			if (!res) {
	...
	// 处理提交后继续添加
	const confirmAndContinue = async (params) => {
		try {
			let formValues = await form.validateFields();
			const params = {
				name: formValues.name,
			...
			setSubmitContinueLoading(false);
		}
	};
	...
	const renderFormItem = (items) => {
		return (
			...
			items.map((item) => {
				return (
					<Col span={item.span ? item.span : 12} key={item.name}>
						<Form.Item
							name={item.name}
				...
				<div className={styles.right}>
					<Space>
						...
						<Button
							...
							disabled={submitContinueLoading}
							style={{
								fontSize: 14,
							}}
